...
def index():
    ...
    orders = db.execute("select * from orders where datetime > datetime('now','-555 hours');
...
def register():
    ...
    if request.method == "POST":
        ...
        db.execute("INSERT INTO users (username, hash, email, venmo, firstname, lastname, phone) VALUES (:username, :password, :email, :venmo, :firstname, :lastname, :phone)",
                   username=username, password=hashpass, email=email, venmo=venmo, firstname=firstname, lastname=lastname, phone=phone)
